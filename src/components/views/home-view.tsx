...
export function HomeView(props: { filings: FilingView[]; leaders: LeaderRow[] }) {
  ...
  const mirrorable = filings.filter((f) => f.trades.some((t) => t.tokenSymbol))
  const [hero, ...rest] = mirrorable.length ? mirrorable : filings
    ...
    <div className="grid gap-10 pt-8 sm:pt-12">
      <section className="grid gap-4">
        ...
        <p className="max-w-2xl text-base leading-relaxed text-muted-foreground sm:text-lg">
          ...
          mirrors the trades into tokenized stocks held in your own wallet, from anywhere outside the US.
        </p>
      </section>
      ...
      <div className="grid gap-10 lg:grid-cols-[minmax(0,1fr)_20rem]">
        <section className="grid content-start gap-4">
          <div className="flex items-baseline justify-between">
            ...
            <Link href="/receipts" className="text-xs text-muted-foreground hover:text-foreground">
              All receipts
            </Link>
          </div>
          {hero ? (
            <FilingReceipt filing={hero} action={<FollowButton source={hero.source} label={`Mirror ${hero.source.name.split(" ").at(-1)}`} />} />
          ) : (
        ...
        <aside className="grid content-start gap-8">
          <section className="grid gap-3">
            <div className="flex items-baseline justify-between">
              <h2 className="text-sm font-semibold">Members to mirror</h2>
